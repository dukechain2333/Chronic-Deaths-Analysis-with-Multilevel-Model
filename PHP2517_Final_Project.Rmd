...
```
```{r}
anova(lmer.model.1, glm.model.3)
```
...
```{r}
# Model 1 is better than model 2, so we remove the random effect of `Other spending per Decedent during the last two years of life` from lmer.model.2
...
```{r}
# Model 3 is better than model 1, so we keep the random effect of `Total ICU Bed Inputs per 1,000 Decedents during the Last Two Years of Life` in lmer.model.3
anova(lmer.model.3, lmer.model.1)
```
...
```
```{r}
# model 3 is better than model 4, so we remove the random effect of `Standardized FTE physician labor: Ratio MS/PC (calculated)` from lmer.model.4
anova(lmer.model.4, lmer.model.3)
```
...
```{r}
# model 5 and model 3 don't have significant difference, so we keep the random effect of `Medical & Surgical Unit Days per Decedent during the Last Six Months of Life` in lmer.model.5
anova(lmer.model.5, lmer.model.3)
```
...
```{r}
# model 6 and model 5 don't have significant difference, so we keep the random effect of `Home Health Agency Visits per Decedent during the Last Six Months of Life` in lmer.model.6
anova(lmer.model.6, lmer.model.5)
```
...
```{r}
# model 6 AIC value is lower than model 7, so we remove the random effect of `Home Health Agency Visits per Decedent during the Last Six Months of Life` in lmer.model.7
anova(lmer.model.7, lmer.model.6)
```

```{r}
lmer.model.best <- lmer.model.6
glm.model.best <- glm.model.3
```
