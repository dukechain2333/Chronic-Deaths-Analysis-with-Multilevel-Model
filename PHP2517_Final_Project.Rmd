...
# Since the column `Hospice Days per Decedent during the Last Six Months of Life` is mislabeled, we will remove it from the data
...
```{r}
#https://www.dartmouthatlas.org/interactive-apps/hospital-care-intensity/
cor(df.chronic$`Percent of enrollees medical inpatient days at hospital to which they were assigned`,df.chronic$`Hospital Care Intensity Index during the Last Two Years of Life`)
```
...
# cor.numeric <- cor(df.numeric, use = "complete.obs")
# cor.numeric
...
```{r}
df.summary<-df.chronic %>% select(-c(Region)) %>%
  tbl_summary(missing="no",
              by = State, 
              type = list(where(is.numeric) ~ "continuous"),
              statistic = list(all_continuous() ~ "{mean} ({sd})")) %>% add_p()%>%
