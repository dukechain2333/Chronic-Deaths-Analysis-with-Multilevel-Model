...
# Traditional Method (Without Variance)
...
This part is based on the model that we obtained in GLM.
...
```{r}
# Model 2 is better than model 1, so we keep the random effect of `Other spending per Decedent during the last two years of life` from lmer.model.2
...
```{r}
# Model 3 is not differnt from model 2, so we remove the random effect of `Total ICU Bed Inputs per 1,000 Decedents during the Last Two Years of Life` from lmer.model.3
