...
```{r}
# vars_to_include <- setdiff(names(df.chronic), c("Hospital Name", "State", "Region"))
# 
# tab <- CreateTableOne(data=df.chronic, vars = vars_to_include, strata="State") 
# kableone(tab)
```
...
```{r}
m.full.1<-lmer(`Number of deaths among chronically ill patients assigned to hospital` ~1+
                       `Other spending per Decedent during the last two years of life`+
               `Total ICU Bed Inputs per 1,000 Decedents during the Last Two Years of Life`+
               `High-Intensity ICU Bed Inputs per 1,000 Decedents during the Last Two Years of Life`+
               `Intermediate-Intensity ICU Bed Inputs per 1,000 Decedents during the Last Two Years of Life`+
...
```{r}
# Remove `High-Intensity ICU Bed Inputs per 1,000 Decedents during the Last Two Years of Life` from m.full.1

m.full.2<-lmer(`Number of deaths among chronically ill patients assigned to hospital` ~1+
                       `Other spending per Decedent during the last two years of life`+
               `Total ICU Bed Inputs per 1,000 Decedents during the Last Two Years of Life`+
               `Intermediate-Intensity ICU Bed Inputs per 1,000 Decedents during the Last Two Years of Life`+
               `SNF Bed Inputs per 1,000 Dededents during the Last Two Years of Life`+
...
```
```{r}
# Remove `Intermediate-Intensity ICU Bed Inputs per 1,000 Decedents during the Last Two Years of Life` from m.full.2

m.full.3<-lmer(`Number of deaths among chronically ill patients assigned to hospital` ~1+
                       `Other spending per Decedent during the last two years of life`+
               `Total ICU Bed Inputs per 1,000 Decedents during the Last Two Years of Life`+
               `SNF Bed Inputs per 1,000 Dededents during the Last Two Years of Life`+
               `Standardized FTE physician labor: Ratio MS/PC (calculated)`+
...
```{r}
anova(m.full.1,m.full.2,m.full.3)
```

```{r}
#(step_res <- step(m.full))
#m.best <- get_model(step_res)
m.best <- m.full.3
```
...
```{r}
m.ranef.hospital<-ranef(m.best)[[1]]
m.ranef.state <- ranef(m.best)[[2]]
```
...
df.chronic.filter <- df.chronic %>% 
  filter(`Hospital Name` %in% row.names(m.ranef.hospital))

m.ranef.hospital.altered <- m.ranef.hospital %>% 
  rename_with(~paste0("RE-", gsub("`", "", .)), everything()) %>%
  rownames_to_column(var = "Hospital Name")

m.ranef.state.altered <- m.ranef.state %>% 
  rename("RE-State" = "(Intercept)") %>%
  rownames_to_column(var = "State")

result_df <- inner_join(df.chronic.filter, m.ranef.hospital.altered, by = "Hospital Name")
result_df <- inner_join(result_df, m.ranef.state.altered, by = "State")
result_df$predicted <- predict(m.best, newdata = df.chronic.filter)
